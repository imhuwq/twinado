...
"""
twinado server.extensions.session

~~~~~~

基于 server.redis.Redis 的 session 插件. 使用前 tornado app 必须挂载 Redis 实例
...
class Session(dict):
    def __init__(self, handler, default_ttl=79200):
        super(Session, self).__init__()
        self.handler = handler
        self.client = handler.application.redis.client
